...
export default function ProgressClient({ initialData }: { initialData: AnyObj }) {
  ...
  return <div className="progress-final">
    ...
    <main className="dfp-wrap page-stack progress-stack">
      ...
      {state && view === 'pm' && <section className="pm-scoreboard-grid" aria-label="Program manager scoreboard">
        {PM_TASKS.map((card, i) => {
          const profile = Array.from(PM_PROFILES as unknown as any[]).find(pm => pm.name === card.name);
          return <article className={i === 0 ? 'pm-score-card winner' : 'pm-score-card'} key={card.name}>
            <div className="pm-card-top">{card.rank ? <Trophy rank={card.rank}/> : <span className="pm-rank spacer"/>}</div>
            <h3>{card.name}</h3>
            <p><b>{card.title}</b>{card.text ? ` ${card.text}` : ''}</p>
            <button onClick={() => setSelectedPM(profile)}><span>About PM</span><strong>→</strong></button>
          </article>;
        })}
      </section>}

      <button className="config-gear" onClick={() => { setDraft(ensureData(data)); setConfigOpen(true); }}>⚙</button>
