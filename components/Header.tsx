...
export default function Header({ active }: { active?: 'repository' | 'story' }) {
  return (
    <header className="dfp-topbar">
      <div className="dfp-wrap dfp-topbar-inner">
        ...
        <nav className="dfp-nav" aria-label="Primary navigation">
          ...
          <Link className={active === 'repository' ? 'nav-pill active' : 'nav-pill'} href="/repository">NGO Discovery</Link>
          <Link className={active === 'story' ? 'nav-pill active' : 'nav-pill'} href="/story-discovery">Story Discovery</Link>
        </nav>
      </div>
    </header>
  );
